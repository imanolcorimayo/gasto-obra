<template>
  <div class="mb-8">
    <div class="max-w-md mx-auto">
      <h1 class="text-[28px] font-bold tracking-tight mb-6">Unirme como cliente</h1>

      <!-- Loading -->
      <AppLoader v-if="isLoading" />

      <!-- Already joined -->
      <div v-else-if="alreadyJoined" class="bg-go-surface rounded-go-xl border border-go-border p-6 text-center">
        <h2 class="text-base font-semibold text-go-success mb-2">Ya sos cliente de este proyecto</h2>
        <p class="text-go-text-tertiary mb-4">{{ project?.name }}</p>
      ...
      <div v-else-if="project" class="bg-go-surface rounded-go-xl border border-go-border p-6">
        <h2 class="text-base font-semibold mb-2">{{ project.name }}</h2>
        <div class="flex flex-col gap-2 text-sm text-go-text-tertiary mb-6">
          <p v-if="project.address">{{ project.address }}</p>
          <p>
            Estado:
            <span :class="project.status === 'active' ? 'text-go-success' : 'text-go-text-tertiary'">
        ...
        <!-- Not authenticated -->
        <div v-if="!user">
          <p class="text-go-text mb-4">Para unirte como cliente, inicia sesion con Google:</p>
        ...
        <div v-else>
          <p class="text-go-text mb-4">
            Sesion iniciada como <span class="text-go-text font-medium">{{ user.email }}</span>
          </p>
          <button
            @click="joinProject"
            :disabled="isJoining"
